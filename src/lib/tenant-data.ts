...
export async function getMonthlyOverview(
  ...
  return {
    ...
    hasIncomeNoExpenses,
    income: { sources: incomeSources, total: totalIncome, salary: salaryAmount },
    expenses: {
      daily: dailyExpenses,
      build: buildExpenses,
      total: totalExpenses,
...
export async function getYearlyTrend(userId: string, year: number) {
  const months = await Promise.all(
    Array.from({ length: 12 }, (_, i) => getMonthlyOverview(userId, year, i + 1))
  );
...
export async function getAnnualShowcaseData(userId: string, year: number) {
  ...
  const monthly = months.map((m) => ({
    ...
    savings: m.savings.contributions,
    net: m.net,
    hasActivity:
      m.income.total > 0 ||
      m.expenses.total > 0 ||
      m.savings.contributions > 0,
  }));
  ...
  return {
    ...
    savingsPlans: savingsPlans.map((plan) => ({
      ...
      status: plan.status,
      annualContribution:
        decimalToNumber(plan.monthlyContribution) * activeMonths.length,
    })),
    salaryAnnual,
    insights,
