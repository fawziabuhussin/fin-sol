...
export async function GET(
  ...
  try {
    ...
    const plan = await getSplitPlan(user.id, planId);
    if (!plan) {
...
export async function PATCH(
  ...
  try {
    ...
    const installmentCount = clampInstallmentCount(
      d.installmentCount ?? plan.installmentCount ?? 1
    );
    ...
    const recurring = recurringFromSplit(
      totalAmount,
      installmentCount,
      firstPaymentAmount
    );
    ...
    await prisma.$transaction(async (tx) => {
      await tx.splitPaymentPlan.update({
        where: { id: planId },
        data: {
          ...(d.title !== undefined ? { title: d.title || plan.title } : {}),
      ...
      if (startChanged || amountsChanged) {
        ...
        const amountMap = amountsBySequence(
          totalAmount,
          firstPaymentAmount,
          installmentCount
        );
        ...
        if (toDelete.length > 0) {
          await tx.splitPaymentInstallment.deleteMany({
        ...
        for (const inst of keepPending) {
          ...
          if (Object.keys(data).length > 0) {
            await tx.splitPaymentInstallment.update({
              where: { id: inst.id },
              data,
            });
          }
        ...
        for (let seq = 1; seq <= installmentCount; seq++) {
          ...
          await tx.splitPaymentInstallment.create({
            data: {
              planId,
              sequence: seq,
              label:
    ...
    const updated = await getSplitPlan(user.id, planId);
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.splitPaymentPlan.delete({ where: { id: planId } });
